...
void Game::game_update(GLFWwindow* window, EngineGL& GL, framebuffer frameB, Manentity_type& man_e, bool iaupdate){
	...
	//systems.input_sys.MoveCamera(man_e, GL);
	...
	//std::cout << "ein " << man_e.getEntityCMP<CHealth>(man_e.getEntityArray()[0]).health/100 << "\n";
...
void Game::update_candil( EngineGL& GL, Manentity_type& man_e)
{
    ...
    //auto& PHY = man_e.getEntityCMP<CPhysics>(ent);
	//std::cout << " lugar aprox " << PHY.x << "  "<< PHY.y << "  "<< PHY.z << "\n";
	if(light.isOn) 
	{
	...
	//td::cout << "no se de mates   "<< man_e.getEntityCMP<CHealth>(man_e.getEntityArray()[2]).health  <<  "y por eso hago esto  "<< (1/(man_e.getEntityCMP<CHealth>(man_e.getEntityArray()[2]).health/100))/100<< "\n";  //50 y 25
	GL.candilResource.setPosition(-0.8f, pos, 0.0f);
	//std::cout << "posicion posicion " << pos << "en base a esta vida " << man_e.getEntityCMP<CHealth>(man_e.getEntityArray()[2]).health<< "\n";
